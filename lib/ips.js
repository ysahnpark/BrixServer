...
module.exports.Ips = function() {
    ...
    this.postSubmission = function(data, callback)
    {
        ...
        .then(function(seqNodeInfo){
            ...
            .then(function(result)
            {
                ...
                ipsReturn.attemptsRemaining = remainingAttempts;

                // @todo - this needs to be able to handle it being not there, null, or false
                if (cePayload.answerKey.nonRecordable)
                {
                    console.log("------ THIS IS NON-RECORDABLE ");
                } else {
     ...
     * numAttemptsAllowed, also accounting for the submission that was just made that hasn't yet
     * been turned into a NodeResult
     * @return {[int]} Number of remaining attempts.
     */
    function calculateRemainingAttempts_(numAttemptsAllowed, sequenceNode)
    {
        // @todo - at some point PM wants unlimited attempts, denoted with a '-1' return value
        // NOTE - This will fail badly if we put anything but submissions into nodeResults

        // "fixed number of attempts" - "number of NodeResults" - "the submission just made"
        return numAttemptsAllowed - sequenceNode.nodeResult.length -1;
    }
