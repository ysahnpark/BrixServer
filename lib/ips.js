...
module.exports.Ips = function() {
        ...
        this.sequenceNodeProvider.getSequenceNodeByKey(data.sequenceNodeKey, function(gsnError, gsnResult){
            ...
            else
            {
                ...
                that_.amsProxy.sendInteraction(data, function(error, result) {
                    ...
                    // @todo - add/update the ResultNode with an interaction-specific node, 
                    // not add to the ResultNode array.  Then pass that in to updateCache_
                    
                    updateCache_(data.sequenceNodeKey, gsnResult);
        ...
        .then(function(seqNodeInfo){
            ...
            .then(function(result){
                ...
                that_.amsProxy.sendSubmission(nodeResult, function(error, result) {
                    ...
                    // FIX THIS - if you've got an error from the AMS, make sure this is doing
                    // what we want.  This'll be easier if we promise this up
                    handlerError_(error);
     ...
     * @param  {[type]} seqNodeInfo {hubSession: <sequenceNodeKey>, seqNodeInfo: {hubSession:<hub sess>, 
     *                              sequenceNodeContent:<value>}}
     * @param  {[type]} nodeResult  A NodeResult
     ...
     * @private
     * 
     * @param  {Object} sequenceNodeKey     Hashed key identifiying a SequenceNode
