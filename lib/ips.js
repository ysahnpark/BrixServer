...
module.exports.Ips = function() {
    ...
    this.retrieveSequenceNode = function(payload, callback) {
        ...
        this.sequenceNodeProvider.getSequenceNode(sequenceNodeIdentifier, function(err, body)  {
            ...
            var snIdfierSum = that_.sequenceNodeProvider.getSequenceNodeIdentifierSummary(sequenceNodeIdentifier);
            if (err !== null) {
                logger.error('SequenceNodeProvider.getSequenceNode with identifier:', snIdfierSum, ' returned ERROR:', err);
                callback(err, body);
            } else {
                logger.debug('SequenceNodeProvider.getSequenceNode with identifier:', snIdfierSum, ' successfully returned: ', body);
                logger.trace('SequenceNode full content: ' + JSON.stringify(body.sequenceNodeContent));
                var targetActivity = body.sequenceNodeContent.targetActivity;
                var nodeResult = body.sequenceNodeContent.nodeResult;
                ...
                // Sanitize the targetActivity by removing the answerKey and adding in state, derived
                // from nodeResult
                var sanitizedTargetActivity = sanitizeBrixConfig_(targetActivity, nodeResult);

                // Add in config information
                sanitizedTargetActivity = addConfigInfo_(sanitizedTargetActivity);

                var itemIds = that_.sequenceNodeProvider.extractItemIds(body.sequenceNodeContent);
    ...
    {
        logger.debug('SequenceNodeProvider.postInteraction called with sequenceNodeKey [' + data.sequenceNodeKey + ']');
        this.sequenceNodeProvider.getSequenceNodeByKey(data.sequenceNodeKey, function(gsnError, seqNodeInfo){
            ...
            if (gsnError)
            {
                logger.error('SequenceNodeProvider.getSequenceNodeByKey with key [', data.sequenceNodeKey, '] returned ERROR:', gsnError);
    ...
    {
        logger.debug('SequenceNodeProvider.postSubmission called with sequenceNodeKey [' + data.sequenceNodeKey + ']');
        ...
        .catch(function (error)
        {
            logger.error('SequenceNodeProvider.getSequenceNodeByKey with key [', data.sequenceNodeKey, '] returned ERROR:', error);
            callback(error, null);
        })
        .done();
    };
    ...
    function updateCache_(sequenceNodeKey, seqNodeInfo)
    {
        ...
        {
            // Using that_ instead of this, as we are in context of callback.
            logger.trace('Updating cache for sequenceNodeKey [' + sequenceNodeKey +'] value:',  seqNodeInfo);
            that_.sequenceNodeProvider.updateSequenceNodeInCache(sequenceNodeKey, seqNodeInfo, function(err, updateResult)  {
                if (err !== null) {
                    logger.error('updateSequenceNodeInCache returned error for sequenceNodeKey ['+ sequenceNodeKey + '] .', error);
                } else {
                    logger.debug('updateSequenceNodeInCache completed for sequenceNodeKey ['+ sequenceNodeKey+ '] result: ', updateResult);
                }
            });
        }
    }
    ...
    /**
     * Builds a state object for submission types, desired by the client, from a nodeResult.
     *
     * NOTE/@todo: This approach has the IPS understanding which properties of the NodeResult
     * make a state object appropriate for a given assessment type, which is surfing on the edge of
     * giving the IPS an understanding of bric types which we do not want.  For all current assessment
     * types this is ok because the state is roughly the same for all of them.  If at some point
     * there's a wildly different state makeup for some new assessment type this model may break down.
     * Further, this model requires that correctAnswer be added to NodeResult, which at this point is
     * unnecessary for all requirements other than state restoration.  At some point we may want to
     * call the CorrectnessEngine to ask for this information rather than store it in NodeResult. 
     * 
     * @param  {Object} nodeResult
     * @return {Object} state
     */
    function buildSubmissionStateObject_(nodeResult)
    {
        var submissions = [];
        ...
        for (var i = 0; i < nodeResult.length; i++)
        {
            var sub = {};
            // submission object
            sub.studentSubmission = nodeResult[i].studentSubmission;
            // correctness, 0..1
            sub.correctness = nodeResult[i].rawItemScore;
            // feedback
            sub.feedback = nodeResult[i].systemResponse.htmlResponse ? nodeResult[i].systemResponse.htmlResponse : "";
            // attempts made
            sub.attemptsMade = i + 1;
            // correct answer object, if available
            if (nodeResult[i].correctAnswer)
            {
                sub.correctAnswer = nodeResult[i].correctAnswer;
            }
    ...
    /**
     * Sanitizes the node content by removing the answerKey section and 
     * adding the state section.
     * We have constrained that there will be only one assessment bric per
     * sequence node, thus only one answerKey.
     * We are adding state (for assessment questions) in place of an
     * answerKey.
     *
     * @param  {Object} targetActivity  The entire value of the target activity
     *                                  property from the sequence node content
     * @param  {Object} nodeResult      The NodeResult object
     * @return {Object} The sanitized activity config 
     */
    function sanitizeBrixConfig_(targetActivity, nodeResult)
    {
        ...
        result['containerConfig'].forEach(function(containerItem, ccKey) {
            ...
            containerItem['brixConfig'].forEach(function(brixItem, bcKey) {
                if (brixItem['answerKey'])
                {
                    delete brixItem['answerKey'];

                    // Create a state object out of the NodeResult and add it in place
                    // of the answerKey.  NodeResult is always supposed to exist but
                    // just in case...
                    // @todo - If we ever want to restore state for non-assessment
                    // brix this tactic won't work.
                    if (nodeResult)
                    {
                        brixItem.state = buildSubmissionStateObject_(nodeResult);
                    }
                }
    ...
    function buildSubmissionNodeResult_(result, studentSubmission, itemCorrelationToken, isLastAttempt)
    {
        ...
        _.extend(nodeResult,
            {
                ...
                "systemResponse": {
                    "htmlResponse": result.data.feedback
                },
        ...
        // Add correctAnswer when available.
        // NOTE/@todo - Eventually we may remove this, instead pulling CorrectAnswer directly from the
        // CE when we want it rather than storing this in NodeResult, as this is used solely for
        // state recovery.
        // NOTE - CE returns "correctAnswer": {}.  We'd prefer to not supply this property if it's empty.
        // correctAnswer - brix-specific object containing the correct answer as returned by the CE.
        if (!(_.isEmpty(result.data.correctAnswer)))
        {
            nodeResult.correctAnswer = result.data.correctAnswer;
        }
    ...
    this.buildSubmissionStateObject__ = buildSubmissionStateObject_;
