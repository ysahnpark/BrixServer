...
module.exports.SequenceNodeProvider = function()
{
    ...
    logger.info('Loading SequenceNodeProvider version ' + VERSION + '.');
...
//console.log("-- FROM CACHE -- "+ sequenceNodeKey);
                        callback(null,
                            {
                                sequenceNodeKey: sequenceNodeKey,
                                itemCorrelationToken: seqNodeInfo.itemCorrelationToken,
                                sequenceNodeContent: seqNodeInfo.sequenceNodeContent,
                                fromCache: true
                            });
                    }, function(error) {
                        ...
                        requestHttp_(reqMethod, reqUrl, reqHeader, reqContent)
                            .then(function(data){
//console.log("-- FROM REMOTE -- "+ sequenceNodeKey);
                                var dataToCache = {
                                ...
                                dataToCache.sequenceNodeContent.targetActivity.sequenceNodeKey = sequenceNodeKey;
                                ...
                                dataToCache.sequenceNodeContent.targetActivity.maxAttempts = 3;

                                storeInCache_(sequenceNodeKey, dataToCache);
                                logger.debug('Sequence Node retrieved from "' + reqUrl + '" and saved to cache:', dataToCache);
                                callback(null, {sequenceNodeKey: sequenceNodeKey, sequenceNodeContent: data.content, fromCache:false});
                            }, function(error) {
                                var errorBody ={};
...
//console.log("RES:" + JSON.stringify(response.headers));
            ...
            if (!error) {
                ...
                if (!response.headers.itemcorrelationtoken)
                {
                    ...
                    errorBody.error = "AMS did not return itemcorrelationtoken in the header";
                    deferred.reject(errorBody);
                }
            ...
            {
                errorBody.statusCode = (response.statusCode >= 300) ? response.statusCode : 500;
                errorBody.response = response;
                errorBody.body = body;
                errorBody.error = error;
                deferred.reject(errorBody);
