...
module.exports.SequenceNodeProvider = function()
{
    ...
    logger.debug('Loading SequenceNodeProvider.');
     ...
     * 
     * @param {Object} sequenceNodeIdentifier   The request parameters in JSON.
    ...
    /***************************************************************************
     *
     * Returns sequenceNodeIdentifier summary, information useful for diagnosis.
     * 
     * @param {Object} sequenceNodeIdentifier   The sequenceNodeIdentifer object.
     *
     * @return {Object}                         Flattened object with selected fields 
     ****************************************************************************/
    this.getSequenceNodeIdentifierSummary = function(sequenceNodeIdentifier)
    {
        var sniSummary = {
            content_targetBinding: sequenceNodeIdentifier.content.targetBinding,
            content_callContext_courseId: sequenceNodeIdentifier.content.callContext.courseId,
            content_callContext_assetId:  sequenceNodeIdentifier.content.callContext.assetId,
        };
        return sniSummary;
    };
    ...
    /***************************************************************************
     *
     * Returns sequenceNodeInfo summary, information useful for diagnosis.
     * Note: sequenceNodeInfo is the envelop data structure used for strong sequence 
     * node information in the cache. 
     * SequenceNodeInfo = {
     *     hubSession: SequenceNodeIdentifier.header['Hub-Session']
     *     itemCorrelationToken: {from AMS response header “itemCorrelationToken”}
     *     sequenceNodeContent: {from AMS response content which is seq node as replied by PAF}
     * }
     * 
     * @param {Object} sequenceNodeInfo   The sequenceNodeInfo object.
     *
     * @return {Object}                         Flattened object with selected fields 
     ****************************************************************************/
    this.getSequenceNodeInfoSummary = function(sequenceNodeKey, sequenceNodeInfo)
    {
        seqNodeInfoSum = {
            sequenceNodeKey: sequenceNodeKey,
            hubSesssion: sequenceNodeInfo.hubSession,
            itemCorrelationToken: sequenceNodeInfo.itemCorrelationToken,
            sn_guid: sequenceNodeInfo.sequenceNodeContent.guid,
            sn_boundActivity: sequenceNodeInfo.sequenceNodeContent.targetBinding ? sequenceNodeInfo.sequenceNodeContent.targetBinding.boundActivity : null,
            sn_user: sequenceNodeInfo.sequenceNodeContent.parentSequence.user,
            sn_learningContext: sequenceNodeInfo.sequenceNodeContent.parentSequence.learningContext,
        };
        return seqNodeInfoSum;
    };

/***************************************************************************
     *
     * Returns the item id information part from the SequenceNodeInfo.
     * The return value structure is:
     *   {
     *     assignment_id : "<assignment-url>",
     *     activity_id : "<activity-url>"
     *   }
     * This implementation obtains the information from the sequenceNodeContent
     * 
     * @param {Object} sequenceNodeContent  The sequenceNode content object
     *                                       as provided by PAF Hub.
     *
     * @return {Object}                   The item id message as defined above. 
     ****************************************************************************/
    this.extractItemIds = function(sequenceNodeContent)
    {
        if (!sequenceNodeContent.targetBinding ||
            !sequenceNodeContent.targetBinding.boundActivity ||
            !sequenceNodeContent.parentSequence ||
            !sequenceNodeContent.parentSequence.toolSettings ||
            !sequenceNodeContent.parentSequence.toolSettings.assignmentUrl)
        {
            logger.error('Missing boundActivity or assignmentUrl properties. Has the server response format changed??');
            return null;
        }

        var itemIds = {
            activity_id: sequenceNodeContent.targetBinding.boundActivity,
            assignment_id: sequenceNodeContent.parentSequence.toolSettings.assignmentUrl
        };
        return itemIds;
    };
            ...
            .then(function(report){
                ...
                if (reqContent['@type'] !== 'SequenceNode') {
                    var error = {
                        ...
                        "cause": 'Wrong type, must be SequenceNode'
                    };
                    callback(error, null);
                    return;
                }
                ...
                retrieveFromCache_(sequenceNodeKey)
                    .then(function(seqNodeInfo){
                        logger.trace('Sequence Node with key ['+sequenceNodeKey+'] retrieved from cache.');
                        ...
                        requestHttp_(reqMethod, reqUrl, reqHeader, reqContent)
                            .then(function(data){
                                ...
                                logger.trace('Sequence Node retrieved from "' + reqUrl + '" and saved to cache.');
...
//console.log("RES:" + JSON.stringify(response.headers));
            ...
            if (error || response.statusCode >= 300)
            {
                ...
                errorPayload.code = (response && response.statusCode >= 300) ? response.statusCode : 500;
                errorPayload.message = (response && response.statusCode >= 300) ? http.STATUS_CODES[response.statusCode] : error;
            ...
            else
            {
                ...
                // [10/31/2013] Due to a bug in PAF the targetActivity contains
                // escaped quotes, requiring second pass of parsing. 
                // To future proof, the double parsing is not done when targetActivity
                // is correctly object.
    ...
    this.getHealth = function(callback) {
        ...
        request.get(options, function (error,response,body) {
            if (error !== null) {
                ...
                body.statusCode = (response) ? response.statusCode: 500;
