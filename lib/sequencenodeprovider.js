    ...
    function requestHttp_(method, url, header, body) {
        ...
        request(options, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                logger.debug('Response from AMS ' + body);
                // @todo: Hub, when not found, is returning "404 Not Found" as body,
                //        handle accordingly.
                var responseInJson = JSON.parse(body);
                ...
                if (_.isString(responseInJson.targetActivity))
                {
                    responseInJson.targetActivity = JSON.parse(responseInJson.targetActivity);
                }
                deferred.resolve(responseInJson);
            }
            else
            {
                var errorBody ={};
                errorBody.statusCode = (response.statusCode > 202) ? response.statusCode : 500;
                errorBody.error = error;
                deferred.reject(errorBody);
            }
        });
