...
module.exports = function(config) 
{
	 ...
	 * 
	 * @param {string} sequenceNodeIdentifier   The request parameter in stringified JSON.
	...
	this.getSequenceNode = function(sequenceNodeIdentifier, callback) {
		...
		var jsonParam = JSON.parse(sequenceNodeIdentifier);
					...
					.then(function(data){
						...
						requestHttp_(reqUrl, reqHeader, reqContent, reqMethod)
							.then(function(data){
								...
								callback(null, {sequenceNodeKey: sequenceNodeKey, sequenceNodeContent: data});
							}, function(error) {
								var errorBody ={};
	 ...
	 * This method retrieves the information from chace, and does not perform 
	...
	this.getSequenceNodeByKey = function(sequenceNodeKey, callback) {
		...
		retrieveFromCache_(sequenceNodeKey)
			.then(function(data) {
				...
				callback('Key not found', 'Key ' + sequenceNodeKey + ' not in the cache.s');
			});
	}
