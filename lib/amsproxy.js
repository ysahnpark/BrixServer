...
module.exports = function(config) {
	 ...
	 *                            where data is result object (parsed JSON)
	 ****************************************************************************/
	this.getSequenceNode = function(reqParams, callback) {
		...
		var sequenceNodeKey = crypto.createHash('md5').update(reqParams).digest("hex");
		validateMessage_(jsonParams, messageSchema)
			.then(function(report){
				// Validation success
console.log("**Validation success");
				retrieveFromCache_(sequenceNodeKey)
					.then(function(data){
console.log("**Cache success {key:"+ sequenceNodeKey + ", body: " + data+"}");
...
console.log("**Cache failed");
						...
						requestPost_(config.amsBaseUrl + '/seqnode', jsonParams)
							.then(function(data){
console.log("**REST access success");
								// Set to the cache
								redisClient.set(sequenceNodeKey, JSON.stringify(data));
								callback(null, {sequenceNodeKey: sequenceNodeKey, data: data});
							}, function(error){
console.log("**REST access fail");
								var errorBody ={};
